...
const feedback = async (req, res) => {
  ...
  const { name, email, contactNumber, state, city, center, ratings, yesNoAnswers, suggestions } = req.body;

  if (!name || !contactNumber || !state || !city || !center) {
    return res.status(400).json({
      success: false,
      messagee: "Please fill all the required fields!"
    });
  };
...
<![endif]-->
  ...
  <body class="body">
    ...
    ${Object.entries(ratings)
        .map(([question, rating]) => `<li><strong>${question}:</strong> ${rating}</li>`)
        .join('')}
  </ul>

  <p style="text-align:left"><strong>✅ Yes/No Answers:</strong></p>
  <ul>
...
const brownField = async (req, res) => {
  ...
  if (!state || !city || !hospital || !name || !phone  || !dialysisCount) {
    return res.status(400).json({
      success: false,
